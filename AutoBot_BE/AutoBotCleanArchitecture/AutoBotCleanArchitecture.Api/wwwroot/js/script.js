// This file is for the Chrome extension/bot runtime (runs inside the trading website).
// It is NOT used by the Next.js frontend.
//
    ...
    try {
        $.ajax({
            ...
            success: (data) => {
                ...
                if (list.length > 0) {
                    list.map((sig) => {
                        // DTO_BotSignal: { id, dateTime, signal, price }
...
const my_logout = () => {
    try {
        // AutoBot_BE: logout is handled by Device/UserLogout
        $.ajax({
...
const refreshToken = async () => {
    try {
        ...
        // AutoBot_BE exposes: POST /api/Authen/RenewAccessToken with { accessToken, refreshToken }
        const data = JSON.stringify({ accessToken: access_token, refreshToken: refresh_token })
        ...
        await $.ajax({
            ...
            const tokenObj = data?.data || data?.Data
            if (tokenObj?.accessToken || tokenObj?.AccessToken) {
                setCookie("auth_token", tokenObj.accessToken || tokenObj.AccessToken, 5);
            }
            if (tokenObj?.refreshToken || tokenObj?.RefreshToken) {
                const user = getCurrentUser()
                if (user) {
                    user.refresh_token = tokenObj.refreshToken || tokenObj.RefreshToken
                    setCookie("bot_data", JSON.stringify(user), 1 * 24 * 60)
                }
            }
        }).fail(() => {
...
window.addEventListener('load', async () => {
    ...
    async function loggingAndBot(isLogin = false, userId) {
        ...
        const botAutoClick = async (arr, fullHopdong = parseInt(botVolumeValue.val()), isAdmin = false) => {
            ...
            // Harden: REVERSE might not always be the last line depending on server formatting
            const daGuiReverse = arr.some(l => l.startsWith("REVERSE")) || arr[arr.length - 1] === "REVERSE" || type[0] === "REVERSE";
            const soViThe = parseInt($('#danhmuc_' + $('#right_stock_cd').val()).children('td').eq(1).html())
        ...
        var connection = new signalR.HubConnectionBuilder()
            // AutoBot_BE hub: app.MapHub<MessageHub>("/messageHub")
