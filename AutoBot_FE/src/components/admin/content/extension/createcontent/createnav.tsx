...
const CreateNav = ({ isOpenModal, existingData, accessToken, setIsOpenModal, onRefresh }: {
    isOpenModal: boolean,
    existingData: any,
    accessToken: string,
    ...
    const onFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (secureUrl) {
                ...
                toast.success("Tải icon điều hướng thành công!");
            }
        } catch (error) {
    ...
    const handleCreateNav = async () => {
        if (!contentData.title.trim()) return toast.error("Vui lòng nhập tên mục điều hướng");
        if (!contentData.icon.trim()) return toast.error("Vui lòng tải lên icon");
        if (!contentData.description.trim()) return toast.error("Vui lòng nhập mô tả hướng dẫn");
        setLoading(true);
        try {
            ...
            toast.success("Thêm điều hướng mở rộng thành công!");
            setContentData({ title: '', description: '', icon: '', otherType: 'nav-extension' });
                        ...
                        <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-[98vw] h-[95vh] overflow-hidden flex flex-col pointer-events-auto border border-gray-100">
                            <div className="flex justify-between items-center p-4 border-b bg-white shrink-0">
                                <h2 className="text-xl font-black flex items-center gap-3 uppercase tracking-tighter text-black">
                                    Điều hướng hệ thống ({existingData?.filter((i: any) => i.otherType === 'nav-extension').length}/4)
                                </h2>
                                <button onClick={() => {
                                    setIsOpenModal(false);
                                    setContentData({ title: '', description: '', icon: '', otherType: 'nav-extension' });
                            ...
                            <div className="flex flex-1 overflow-hidden bg-gray-50">
                                <div className="w-[420px] p-8 space-y-6 border-r overflow-y-auto bg-white shrink-0 scrollbar-hide">
                                    <div className="space-y-5">
                                        <div>
                                            <label className="block text-[.75rem] font-black mb-1.5 text-blue-600 uppercase tracking-widest">Tiêu đề điều hướng</label>
                                            <input type="text" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm outline-none font-bold focus:border-indigo-500 transition-colors" value={contentData.title} onChange={(e) => setContentData({ ...contentData, title: e.target.value })} placeholder="VD: TRUNG TÂM HỖ TRỢ" />
                                        </div>
                                        <div>
                                            ...
                                            {!contentData.icon ? (
                                                <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="w-full h-40 border-2 border-dashed border-gray-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-3 text-gray-400 hover:border-indigo-400 hover:bg-indigo-50 transition-all cursor-pointer">
                                                    ...
                                                    <span className="text-[10px] font-black uppercase tracking-tight text-center px-6">Tải lên icon điều hướng</span>
                                                </button>
                                            ) : (
                                        ...
                                        <div>
                                            <label className="block text-[.75rem] font-black mb-1.5 text-blue-600 uppercase tracking-widest">Mô tả ngắn gọn</label>
                                            <textarea rows={6} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm outline-none resize-none font-medium text-gray-600 leading-relaxed focus:border-indigo-500 transition-colors" value={contentData.description} onChange={(e) => setContentData({ ...contentData, description: e.target.value })} placeholder="Nhập mục đích của menu điều hướng này..." />
                                        </div>
                                    </div>
                                    <div className="pt-6 space-y-3">
                                        <button
                                            onClick={handleCreateNav}
                                            disabled={loading || uploading || (existingData?.filter((i: any) => i.otherType === 'nav-extension').length >= 4)}
                                        ...
                                        >
                                            ...
                                            {loading ? "Đang xử lý..." : "Lưu điều hướng"}
                                        </button>
                                        <button onClick={() => {
                                            setIsOpenModal(false);
                                            setContentData({ title: '', description: '', icon: '', otherType: 'nav-extension' });
                                                ...
                                                >
                                                    <div className={`text-center transition-all ${viewMode === 'mobile' ? 'pt-16 pb-12' : 'pt-24 pb-20'}`}>
                                                        <h1 className={`font-black italic uppercase tracking-tighter text-slate-900 leading-none transition-all ${viewMode === 'mobile' ? 'text-5xl' : 'text-8xl'}`}>
                                                            <span className="text-indigo-600">Extension</span>
                                                        </h1>
                                                    </div>
                                                    <div className="px-14 pb-32">
                                                        <div className={`grid ${config.columns} gap-8`}>
                                                            {existingData
                                                                ?.filter((i: any) => i.otherType === 'nav-extension').length < 4 && (
                                                                    <NavCardItem data={contentData} isDraft={true} viewMode={viewMode} />
                                                                )}
                                                            {existingData
                                                                ?.filter((i: any) => i.otherType === 'nav-extension')
                                                                .map((item: any, idx: number) => (
                                                                    <NavCardItem key={idx} data={item} isDraft={false} viewMode={viewMode} />
                                                                ))
                                                            }
                                                        </div>
                                                    </div>
...
const NavCardItem = ({ data, isDraft = false, viewMode }: any) => {
    return (
        <div className={`w-full rounded-[3.5rem] p-10 flex flex-col items-center text-center shadow-2xl relative overflow-hidden group transition-all duration-500 h-full ${isDraft
            ...
            <div className="w-24 h-24 bg-white rounded-4xl flex items-center justify-center mb-8 shadow-xl p-5 relative z-10 transition-transform group-hover:scale-110">
                {data.icon ? (
                    <img src={data.icon} className="w-full h-full object-contain" alt="nav-icon" />
                ) : (
                    <MdExplore className="text-indigo-600" size={40} />
                )}
            </div>
            <h3 className={`font-black text-white uppercase tracking-tight mb-4 leading-tight relative z-10 ${viewMode === 'mobile' ? 'text-xl' : 'text-2xl'}`}>
                {data.title || "TÊN ĐIỀU HƯỚNG"}
            </h3>
            <p className={`text-slate-400 font-medium leading-relaxed relative z-10 line-clamp-4 ${viewMode === 'mobile' ? 'text-xs' : 'text-sm'}`}>
                {data.description || "Mô tả vai trò của liên kết điều hướng này đối với trải nghiệm khách hàng..."}
            </p>
